...
async def _run() -> None:
    ...
    # Startup tracker — collects events for the terminal dashboard.
    from nekofetch.ui.terminal import StartupTracker, render_startup_dashboard

    tracker = StartupTracker(version=_version(), build=build)
    container.startup_tracker = tracker  # type: ignore[attr-defined]
    ...
    try:
        await manager.start()
        render_startup_dashboard(tracker)
        await stop.wait()
    finally:
        await manager.stop()
