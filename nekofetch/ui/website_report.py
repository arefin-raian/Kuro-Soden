...
def render_report(report: WebsiteReport) -> str:
    ...
    # Coerce defensively: legacy/migrated requests may carry a non-int season count
    # (e.g. a string), and a bare ``> 1`` on that raises TypeError — which used to
    # silently kill the whole report render.
    try:
        seasons = int(report.anilist_seasons or 0)
    except (TypeError, ValueError):
        seasons = 0
    if seasons > 1:
        rows.append(t(M.WEB_REPORT_EXPECTS, eps=eps, seasons=seasons))
    else:
