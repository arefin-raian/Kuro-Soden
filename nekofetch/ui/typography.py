...
def user_label(user) -> str:
    """Human-readable label for a user: ``@username`` → first name → Telegram id.

    Duck-typed so it works for a DB ``User`` (``.telegram_id``) *and* a Pyrogram
    ``User`` (``.id``). It NEVER returns the internal DB primary key — pass the
    user object, not ``request.user_id``.
    """
    if user is None:
        return "—"
    ...
    if first:
        return first
    uid = getattr(user, "telegram_id", None) or getattr(user, "id", None)
