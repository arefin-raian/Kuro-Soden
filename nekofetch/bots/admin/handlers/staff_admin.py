from __future__ import annotations

from pyrogram import Client, filters
from pyrogram.enums import ParseMode
from pyrogram.types import CallbackQuery, Message

from nekofetch.bots.fsm import FSM
from nekofetch.core.container import Container
from nekofetch.core.exceptions import NekoFetchError
from nekofetch.domain.enums import Permission
from nekofetch.localization.messages import M
from nekofetch.services.auth_service import AuthService
from nekofetch.services.staff_service import StaffService
from nekofetch.ui.components import cb, keyboard
from nekofetch.ui.screens import show
from nekofetch.ui.typography import user_label

STATE_ADD = "staff:await_id"


def register(client: Client, container: Container) -> None:
    auth = AuthService(container)
    fsm = FSM(container.redis, bot="admin")
    L = container.localizer.get

    def _can(obj, perm: Permission) -> bool:
        user = getattr(obj, "nf_user", None)
        return bool(user and auth.has_permission(user, perm))

    @client.on_callback_query(filters.regex(r"^admin\|staff"))
    async def _panel(_: Client, q: CallbackQuery) -> None:
        if not _can(q, Permission.MANAGE_STAFF):
            await q.answer(L(M.ACCESS_DENIED), show_alert=True)
            return
        await q.answer()
        await _render(q)

    async def _render(q: CallbackQuery) -> None:
        team = await StaffService(container).list_team()
        rows = []
        if team:
            lines = []
            for m in team:
                dot = L(M.STAFF_MEMBER_DOT_BANNED) if m.banned else L(M.STAFF_MEMBER_DOT_ACTIVE)
                flag = L(M.STAFF_FLAG_BANNED) if m.banned else ""
                lines.append(L(M.STAFF_MEMBER_ROW, dot=dot, name=m.name, role=m.role, flag=flag))
                if m.role != "admin":
                    rows.append([
                        (L(M.STAFF_BTN_REMOVE, name=m.name[:14]), cb("staff", "rm", m.telegram_id)),
                        (L(M.STAFF_BTN_UNBAN) if m.banned else L(M.STAFF_BTN_BAN),
                         cb("staff", "ban", m.telegram_id, 0 if m.banned else 1)),
                    ])
            body = "\n".join(lines)
        else:
            body = L(M.STAFF_EMPTY)
        rows.append([(L(M.STAFF_BTN_ADD), cb("staff", "add"))])
        rows.append([(L(M.BTN_BACK), cb("admin", "home"))])
        await show(client, q.message, f"{L(M.STAFF_TITLE)}\n\n{body}", keyboard(*rows))

    @client.on_callback_query(filters.regex(r"^staff\|add"))
    async def _add(_: Client, q: CallbackQuery) -> None:
        if not _can(q, Permission.MANAGE_STAFF):
            await q.answer(L(M.ACCESS_DENIED), show_alert=True)
            return
        await fsm.set(q.from_user.id, STATE_ADD)
        await q.answer()
        kb = keyboard([(L(M.BTN_BACK), cb("admin", "staff"))])
        await show(client, q.message, L(M.STAFF_ADD_PROMPT), kb)

    @client.on_callback_query(filters.regex(r"^staff\|rm"))
    async def _remove(_: Client, q: CallbackQuery) -> None:
        if not _can(q, Permission.MANAGE_STAFF):
            await q.answer(L(M.ACCESS_DENIED), show_alert=True)
            return
        target = int(q.data.split("|")[2])
        try:
            await StaffService(container).remove_staff(q.from_user.id, target)
            await q.answer(L(M.STAFF_TOAST_DEMOTED))
        except NekoFetchError as exc:
            await q.answer(exc.detail or L(M.ERR_GENERIC), show_alert=True)
            return
        await _render(q)

    @client.on_callback_query(filters.regex(r"^staff\|ban"))
    async def _ban(_: Client, q: CallbackQuery) -> None:
        if not _can(q, Permission.APPROVE_USERS):
            await q.answer(L(M.ACCESS_DENIED), show_alert=True)
            return
        parts = q.data.split("|")
        target, banned = int(parts[2]), bool(int(parts[3]))
        try:
            await StaffService(container).set_banned(q.from_user.id, target, banned)
            await q.answer(L(M.STAFF_TOAST_BANNED) if banned else L(M.STAFF_TOAST_UNBANNED))
        except NekoFetchError as exc:
            await q.answer(exc.detail or L(M.ERR_GENERIC), show_alert=True)
            return
        await _render(q)

    @client.on_message(filters.text & filters.private & ~filters.command(["start"]), group=6)
    async def _add_input(_: Client, message: Message) -> None:
        if not message.from_user:
            return
        state, _ = await fsm.get(message.from_user.id)
        if state != STATE_ADD or not _can(message, Permission.MANAGE_STAFF):
            return
        await fsm.clear(message.from_user.id)
        raw = message.text.strip()
        if not raw.lstrip("-").isdigit():
            await message.reply(L(M.STAFF_ADD_BAD_ID), parse_mode=ParseMode.HTML)
            return
        # Resolve the Telegram profile so the DB row + lists/buttons carry a real
        # name instead of a bare id. Unknown/never-seen id → fall back to the number.
        tg = None
        try:
            tg = await client.get_users(int(raw))
        except Exception:  # noqa: BLE001
            pass
        try:
            await StaffService(container).add_staff(
                message.from_user.id, int(raw),
                username=getattr(tg, "username", None),
                first_name=getattr(tg, "first_name", None),
            )
        except NekoFetchError as exc:
            await message.reply(exc.detail or L(M.ERR_GENERIC), parse_mode=ParseMode.HTML)
            return
        await message.reply(
            L(M.STAFF_ADD_DONE, id=user_label(tg) if tg else raw),
            parse_mode=ParseMode.HTML,
        )
