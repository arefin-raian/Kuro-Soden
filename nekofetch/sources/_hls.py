...
async def download_hls_ts(
    ...
    *,
    ...
    # ── Single pass through the playlist → (segment_url, key_url, iv) ──
    # Some CDNs (AniZone's seiryuu.vid-cdn.xyz) rotate keys mid-stream: each
    # #EXT-X-KEY line applies to every segment that follows it until the next
    # #EXT-X-KEY. A naive parser that grabs only the LAST key/IV would decrypt
    # early segments with the wrong key → garbage → TS integrity failure.
    segs: list[str] = []
    seg_crypto: list[tuple[str | None, bytes | None]] = []
    ...
    for line in media_txt.splitlines():
        ...
        if line.startswith("#EXT-X-KEY:"):
            ...
            if method != "AES-128":
                # #EXT-X-KEY:METHOD=NONE turns encryption off for following segs.
                current_key_url, current_iv = None, None
                continue
            uri_m = re.search(r'URI="([^"]+)"', line)
            current_key_url = urljoin(media_url, uri_m.group(1)) if uri_m else None
            iv_m = re.search(r"IV=0x([0-9a-fA-F]+)", line)
            current_iv = bytes.fromhex(iv_m.group(1)) if iv_m else None
            continue
        # Skip every other tag (#EXTINF, #EXT-X-DISCONTINUITY, etc.).
        if line.startswith("#") or not line.strip():
            continue
        segs.append(urljoin(media_url, line.strip()))
        seg_crypto.append((current_key_url, current_iv))

    if not segs:
        raise RuntimeError("playlist contained no segments")

    # ── Fetch every unique AES key ONCE, in parallel ──
    keys: dict[str, bytes] = {}
    unique_key_urls = {u for u, _ in seg_crypto if u}
    if unique_key_urls:
        key_results = await asyncio.gather(
            *(http.get(u, headers=headers) for u in unique_key_urls),
            return_exceptions=True,
        )
        for u, r in zip(unique_key_urls, key_results):
            if isinstance(r, Exception):
                raise RuntimeError(
                    f"failed to download AES-128 key from {u}: {r}"
                ) from r
            try:
                r.raise_for_status()
            except httpx.HTTPStatusError as exc:
                raise RuntimeError(
                    f"AES-128 key fetch returned HTTP "
                    f"{exc.response.status_code}: {u}"
                ) from exc
            if len(r.content) != 16:
                raise RuntimeError(
                    f"AES-128 key from {u} has unexpected length: "
                    f"{len(r.content)} (expected 16)"
                )
            keys[u] = r.content
    ...
    async def grab(i: int, seg_url: str) -> tuple[int, bytes]:
        ...
        async with sem:
            ...
            # Decrypt AES-128-CBC if encryption is present for THIS segment.
            # Per-segment (key, iv) map handles #EXT-X-KEY rotation mid-stream:
            # segments 0-16 may use key A, segments 17+ key B, etc. Using the
            # wrong key on later segments decrypts to garbage → TS sync fails.
            key_url, iv_explicit = seg_crypto[i]
            aes_key = keys.get(key_url) if key_url else None
            if aes_key:
                # Per the HLS spec §6.2.3: when no explicit EXT-X-KEY:IV, the
                # IV is the media sequence number as a 128-bit big-endian.
                iv = iv_explicit if iv_explicit else (media_seq + i).to_bytes(16, "big")
                cipher = Cipher(algorithms.AES(aes_key), modes.CBC(iv))
                decryptor = cipher.decryptor()
                raw = decryptor.update(raw) + decryptor.finalize()
                # Strip PKCS#7 padding — ALL padding bytes must equal the pad
                # length (e.g. 5 bytes of 0x05), not just the last one.
                pad_len = raw[-1]
    ...
    if not ts_is_clean(clean):
        # Recovery: some origins (e.g. AniZone's CDN) serve segments whose
        # mask is not detected by ts_start() — the per-segment search finds
        # a false-positive offset or misses the mask entirely, leaving junk
        # bytes that shift the sync grid.  Search the assembled stream itself
        # for a clean sync point; trim to it and re-validate.
        clean_bytes = bytes(clean)
        off = ts_start(clean_bytes)
        if off > 0:
            trimmed = clean_bytes[off:]
            if ts_is_clean(trimmed):
                clean = bytearray(trimmed)
        if not ts_is_clean(bytes(clean)):
