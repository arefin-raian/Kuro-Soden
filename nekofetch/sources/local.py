...
class LocalFileSource(AnimeSource):
    ...
    async def download(
        ...
        *,
        ...
        if complete:
            if already:
                # A resumed copy only hashed the bytes written THIS run, so the
                # incremental digest is meaningless. Re-hash the finished file
                # from 0 — otherwise the resumed file returns checksum=None and
                # the verify stage silently skips it (a corrupt resume is never
                # caught).
                full = hashlib.sha256()
                async with aiofiles.open(dest, "rb") as fverify:
                    while True:
                        block = await fverify.read(chunk or 1024 * 1024)
                        if not block:
                            break
                        full.update(block)
