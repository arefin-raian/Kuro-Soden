...
class KickAssAnimeSource(AnimeSource):
    ...
    async def search(self, query: str) -> list[AnimeStub]:
        ...
        # ── fuzzy search with retries + backoff ──────────────────────────
        payload = json.dumps({"page": 1, "query": query})
        headers = {
            "Content-Type": "application/json",
            "Referer": f"{BASE_URL}/search?q={urllib.parse.quote(query)}",
            "Origin": BASE_URL,
        }
        ...
        for attempt in range(3):
            try:
                resp = await self.http.post(
                    f"{BASE_URL}/api/fsearch", content=payload, headers=headers,
                )
                resp.raise_for_status()
                data = resp.json()
        ...
        log.warning("kickass.search.failed", query=query, attempts=3, last=last_error)
        return []
