...
async def apply_bot_branding(container: Container, client, anime_doc_id: str) -> None:
    ...
    from nekofetch.services.bot_naming import format_bot_name

    facts = await MainChannelService(container).gather_facts(anime_doc_id)
    # Build the new-style formatted name.
    qualities = [q.strip() for q in facts.qualities.replace(",", " ").split() if q.strip()]
    name = format_bot_name(
        facts.title, None, audios=set(), languages=None, qualities=qualities,
    )
    if len(name) > 64:
        name = name[:64]

    short = _truncate(
        f"{facts.title} — {facts.qualities} — {facts.languages}", 120,
    )
