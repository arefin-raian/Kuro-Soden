...
async def build_website_report(container: Container, *, title: str,
                               franchise: dict,
                               skip_anizone: bool = False) -> WebsiteReport:
    ...
    # Match on English, Romaji, AND synonyms so the site result is provably the
    # same show — KickAssAnime may only index the Romaji title.
    from nekofetch.sources.telegram.matching import meaningful_variants

    titles = [t for t in (franchise.get("english") or title, franchise.get("romaji")) if t]
    synonyms = franchise.get("synonyms") or []
    titles += [s for s in meaningful_variants(synonyms) if s not in titles]
    ...
    report.coverages = list(await asyncio.gather(
        *[_cov(s) for s in sources_to_check]
    ))
