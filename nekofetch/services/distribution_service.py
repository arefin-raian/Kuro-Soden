...
class DistributionService:
    ...
    async def redeem(self, token: str) -> dict:
        ...
        async with session_scope(self._c.pg_sessionmaker) as session:
            # Lock the row: without it two simultaneous taps both read uses < max,
            # both increment, and both get delivered — bypassing the use cap
            # (over-delivery / revenue leak). FOR UPDATE serializes the redeem.
            link = (
                await session.execute(
                    select(AccessLink).where(AccessLink.token == token).with_for_update()
                )
            ).scalar_one_or_none()
