...
class AccessService:
    ...
    async def redeem(self, token: str, telegram_id: int) -> datetime:
        ...
        async with session_scope(self._c.pg_sessionmaker) as session:
            # Lock the token row so a double-tap on the same link can't be redeemed
            # twice (both reads seeing used=False before either commits).
            row = (
                await session.execute(
                    select(AccessToken).where(AccessToken.token == token).with_for_update()
                )
            ).scalar_one_or_none()
